...
procedure Mark5 is
   ...
   exception
   when Tasking_Error =>
      Printer.printing("Markt is closed!");
      --raise Tasking_Error;
  ...
  protected body Managers is
      procedure incress is
      begin
         num := num+1;
      end incress;
...
begin
   ...
   delay 1.0;
   for i in 1..2 loop
      if(not sport_stores(i).all'Terminated)then
         sport_stores(i).all.Close;
      end if;
      if(not shoes_stores(i).all'Terminated) then
         shoes_stores(i).all.Close;
      end if;

      if(not electronics_stores(i).all'Terminated) then
         electronics_stores(i).all.Close;
      end if;

   end loop;
   Printer.printing("Served " & Integer'Image(Managers.getNum)&" customers");
