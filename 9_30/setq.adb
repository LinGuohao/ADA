...
package body Setq is
   ...
   function "+" ( X, Y: Rational ) return Rational is
   begin
      return Normalize(X.Numerator * Y.Denominator + X.Denominator * Y.Numerator,X.Denominator*Y.Numerator);
   end;
   function "+" ( X: Rational; Y: Positive ) return Rational is
   begin
      return Normalize(X.Denominator * Y + X.Numerator , X.Denominator);
   end;
